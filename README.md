...
5. Eq.(17), right-most term (direct solar beam bouncing) is missing the factor of pi/mu0. See Eqs.(3), where the factor is present. The code also accounts fro this factor properly (see the Aerosol-over-Lambertian surface test).

6. Typo: p.12, after Eq.(15), the Wolfram website is assigned the footnote #16, but the correct one is #17 (p.11) or #19 (p.12) - the two weblinks are identical.

7. Sec.4.2 and Figs.16(ab): splitting atmopshere into equally thick element layeres 'dtau' for integration over optical thicknes 'tau' works well for simulating signal reflected from TOA or tranmitted through BOA. See, e.g., Table 1 (column NL - number of optical layeres - cases with NL > 1) in http://dx.doi.org/10.1016/j.jqsrt.2017.04.035. However, for simulation of light scattering inside atmopshere, the alternation of exticntion profile - Fig.16(b) - may introduce errors. For a slightly different strategy, see 'splittau.py' in 'gsit_multilayer' (reference above).
...
9. Fig.12(c): consider precomputing the arrays mug and wg in lines 14-16 and pass them as parameteres.

10. Fig.12(i) lines 64-67, Fig.12(j) lines 78-83, Fig.13(c) line 32, Fig.13(e) line 41, Fig.14 lines 44-64: in all these cases, we explicitely code the first step in the loop pursuing clarity. This step is, however, optional: including the first step in the main loop would make the code shorter.

...
4. Eq.(17), right-most term (direct solar beam bouncing) is missing the factor of pi/mu0. See Eqs.(3), where the factor is present. The code also accounts for this factor properly (see the Aerosol-over-Lambertian surface test).
...
8. Fig.12(c): consider precomputing the arrays mug and wg in lines 14-16 and pass them as parameters.

9. Fig.12(i) lines 64-67, Fig.12(j) lines 78-83, Fig.13(c) line 32, Fig.13(e) line 41, Fig.14 lines 44-64: in all these cases, we explicitly code the first step in the loop pursuing clarity. This step is, however, optional: including the first step in the main loop would make the code shorter.
